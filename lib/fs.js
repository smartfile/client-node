    ...
    try {
      switch(type) {
        ...
        */
        ...
        case MODE_WRITE:
        case MODE_ALLOWED:
        file = new FileProxy(this, path, mode, callback);
        break;

        default:
        throw new Error(`invalid mode type: ${mode} (${type})`);
        break;
      }
    } catch (e) {
      callback(e);
...
class FileProxy {
  ...
  read(len, callback, offs) {
    ...
    fs.read(this.fd, buffer, 0, len, offs, (e, read, buffer) => {
      ...
      /* Actually, this happens whenever you read the last bit of the file.
      if (read != len) {
        callback(new Error(`could not read all the bytes ${read} < ${le}`));
      }
      */

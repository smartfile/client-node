...
class FileProxy {
  ...
  constructor(sffs, path, flags, accessType, callback) {
    ...
    if (this.accessType === ACCESS_WRITE) {
      // Just open a new file for writing.
      ...
      // First download the file, then open for reading.
      // NOTE: We use discardDescriptor, which closes the file, we can then
      // open the path as a stream for downloading.
      tmp.file({ discardDescriptor: true }, (e, path) => {
        if (e) {
          return callback(e);
        }
  ...
  close(callback, abort) {
    ...
    function closeTemp() {
        fs.close(self.tmpFd, (e) => {
          if (e) {
            // An error closing the temp file is non-fatal, but should be
            // logged.
            logger.error(`Error closing fd ${self.tmpFd}`, e);
          }
        });
    };
    ...
    if (!abort && this.accessType !== ACCESS_READ) {
      ...
      // access type is read, so just free the temp file.
      closeTemp();
      callback(null);
    }
  }

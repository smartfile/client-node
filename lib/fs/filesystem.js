...
class FileSystem {
  ...
  readdir(path, callback, opts) {
    ...
    if (opts && opts.limit) {
      reqOptions.qs['children.limit'] = opts.limit;
    }
  ...
  readdirstats(path, callback, opts) {
    ...
    // Make API call.
    this.rest.info(path, (e, children, json) => {
      if (e) {
        callback(e);
        end({ status: 'error' });
        return;
      ...
      if (json) {
        ...
        this._addCache(clone.path, clone, 1, 2);
      }
      ...
      for (let i = 0; i < children.length; i++) {
        if (!reqOptions.incremental) {
          infos.push(children[i]);
        }

        this._addCache(children[i].path, children[i], 1, 2);
      }

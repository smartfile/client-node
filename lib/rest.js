...
class Client {
  ...
  requestJSON(options, callback) {
    ...
    return this.request(options, (e, r, body) => {
      ...
      if (429 == r.statusCode) {
        ...
        logger.warn(`API 'X-Throttle: ${header}' retry in: ${time}ms`);
      ...
      function poll() {
        self.requestJSON(pollOptions, (e, r, json) => {
          ...
          if (!json.result) {
            ...
            pollInterval = Math.min(pollInterval + 128, 30720);
  ...
  info(p, callback, args) {
    let options = {
      method: 'GET',
      uri: `/api/2/path/info${normPath(p)}`,
    };
  ...
  download(p, callback) {
    const options = {
      method: 'GET',
      uri: `/api/2/path/data${normPath(p)}`,
    };
  ...
  upload(p, stream, callback) {
    ...
    const options = {
      method: 'POST',
      uri: `/api/2/path/data${normPath(dirname)}`,
      formData: data,
  ...
  mkdir(p, callback) {
    const options = {
      method: 'PUT',
      uri: `/api/2/path/oper/mkdir${normPath(p)}`,
    };

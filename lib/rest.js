  ...
  requestJSON(options, callback) {
    ...
    return this.request(options, (e, r, body) => {
      ...
      if (429 == r.statusCode) {
        ...
        logger.warn(`Hit API throttle limit, waiting for ${time}s`);
        setTimeout(this.request, time, options, callback);
        return;

      } else if (199 > r.statusCode || r.statusCode > 300) {
        const e = Error(`url: ${r.url} returned non 2XX: ${r.statusCode}`);

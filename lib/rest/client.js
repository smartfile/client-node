...
class Client {
  ...
  info(p, callback, _options) {
    ...
    function getInfoPage(client, opts, offset) {
      pollCount += 1;
      const pageOptions = {
        ...opts,
        qs: {
          fields: 'children',
          'children.limit': options.qs['children.limit'] || 512,
        },
      };
      if (offset) {
        pageOptions.qs['children.offset'] = offset;
      }
      ...
      client.requestJson(pageOptions, (e, r, json) => {
        ...
        // cancel the listing.
        const children = json.children.results || [];
        // eslint-disable-next-line prefer-destructuring
        const limit = json.children.limit;
        ...
        // in parallel.
        if (limit && json.children.count === limit) {
          // Full page.
          // eslint-disable-next-line no-param-reassign
          offset = json.children.offset + limit;
          logger.debug(`getting items ${offset}-${offset + limit - 1}`);
          setImmediate(getInfoPage, client, pageOptions, offset);
        }
    ...
    if (options.qs && options.qs.fields && options.qs.fields.indexOf('children') !== -1) {

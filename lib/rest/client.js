...
>>>>>>> d3e3b63ad0521da0a375cff06896117597067858:lib/rest/client.js
      const pollOptions = {
        method: 'GET',
      ...
      function poll() {
        self.requestJSON(pollOptions, (req1Error, r1, json1) => {
          ...
          if (!json1.result) {
            callback(Error(`Invalid response: ${JSON.stringify(json1)}`));
          } else if (json1.result.status === 'PENDING'
                     || json1.result.status === 'PROGRESS') {
          ...
          } else if (json1.result.status === 'FAILURE') {
            callback(new Error(`task failed: ${json1.result.result.errors}`));
          } else if (json1.result.status === 'SUCCESS') {
